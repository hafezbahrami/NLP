...
def ValidRomanNumerals(string):
     #ref: https://www.geeksforgeeks.org/validating-roman-numerals-using-regular-expression/
...
def augment_the_traning_dataset(examples: List[str]):
     const_augmentation = "I is 1, V is 5, X is 10, L is 50, C is 100, D is 500, and M is 1000."
     for i, ex in enumerate(examples):
         roman_number: bool = False
         roman_numbers = []
         for word in ex.split(" "):
             if ValidRomanNumerals(word) and (word != "I" and word != "" and word != "\n"):
                 roman_number = True
                 roman_numbers.append(word)
                 if roman_number:
                     for num in roman_numbers:
                         additional_text = num + " is a Greek digit and is identical to " + str(roman_to_int(num)) + "."
                         examples[i] += additional_text
                         examples[i] += " Also, in general, " + const_augmentation
     return examples
