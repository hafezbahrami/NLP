...
class TransformerModel(nn.Module):
    ...
    def __init__(self, ntoken:int, d_model:int, nhead:int, d_hid:int,
        ...
        self.decoder = nn.Linear(d_model, ntoken) # classifier layer: ntoken would e # of classes
    ...
    def forward(self, src: Tensor, src_mask: Tensor) -> Tensor:
        ...
        """
        src = self.encoder(src) * math.sqrt(self.d_model) # embed the input: shape 35X20Xembedding_size  --> embedding_size=200
        src = self.pos_encoder(src)
        output = self.transformer_encoder(src, src_mask)  # shape 35X20X200
        output = self.decoder(output)  # linear classification layer: 28782 words/classes -> shape: 35X20X28782
        return output
