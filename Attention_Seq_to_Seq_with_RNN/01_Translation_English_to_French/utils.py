...
# Beam_v2 is very similar to Beam class above, except it added "hidden_state" and "path" as properties
# that can be added. These 2 params are also added as aditional input-arguments into the add method below.
# Also a new method called "def all_EOS" is added to check if all the sentences in the beam are ended with
# EOS or not.
class Beam_v2(object):
    def __init__(self, size):
        self.size = size
        self.elts = []
        self.scores = []
