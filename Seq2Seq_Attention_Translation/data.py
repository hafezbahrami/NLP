...
def load_datasets(train_path: str, dev_path: str, test_path: str, domain=None, task: str="parsing", min_sent_len: int=1,
                  max_sent_len: int=100, reverse: bool=False) -> (List[Tuple[str,str]], List[Tuple[str,str]], List[Tuple[str,str]]):
    """
    ...
    if task == "parsing":
        ...
        train_raw = load_translation_dataset(train_path, min_sent_len, max_sent_len, reverse)
    #     dev_raw = load_translation_dataset(dev_path, min_sent_len, max_sent_len, reverse)
    #     test_raw = load_translation_dataset(test_path, min_sent_len, max_sent_len, reverse)
        dev_raw = train_raw
        test_raw = train_raw
...
def load_translation_datasets(train_path: str, min_sentence_len: int=1, max_sentence_len: int=100, reverse: bool=False) -> (List[Tuple[str,str]]):
    """
    Reads the training, dev, and test data from the corresponding files.
    :param train_path:
    :param dev_path:
    ...
    :return:
    """
    train_raw = load_translation_dataset(train_path, min_sentence_len, max_sentence_len, reverse)
    return train_raw
...
def load_translation_dataset(filename, min_sentence_len, max_sentence_len, reverse):
    """Reading translation data lines..."""
    ...
    filtered_pairs = []
    for pair in pairs:
        if (min_sentence_len <= len(pair[0].split(" ")) <= max_sentence_len) and min_sentence_len <= len(pair[1].split(" ")) <= max_sentence_len:
            filtered_pairs.append([pair[0], pair[1]])
    if reverse:
        filtered_pairs = [list(reversed(p)) for p in filtered_pairs]
